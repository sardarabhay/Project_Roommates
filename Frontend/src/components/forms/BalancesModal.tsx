...
import type { User, UseBalancesReturn } from '../../types';
...
const BalancesModal = ({ onClose, balances, onSettle }: BalancesModalProps): JSX.Element => {
  ...
  const handleSettle = async (userId: string): Promise<void> => {
    setSettling(userId);
    try {
      await expensesApi.settleWithUser(Number(userId));
