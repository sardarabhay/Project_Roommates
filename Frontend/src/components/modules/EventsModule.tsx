...
import type { User, HouseEvent } from '../../types';
...
const EventsModule = ({ onCreateEvent, user, refreshKey }: EventsModuleProps): JSX.Element => {
  ...
  const handleRSVP = async (eventId: number, response: RSVPStatus): Promise<void> => {
    try {
  ...
  const getRSVPStatus = (event: HouseEvent): RSVPStatus => {
  ...
  const getRSVPCounts = (event: HouseEvent): { going: number; maybe: number; total: number } => {
    return {
